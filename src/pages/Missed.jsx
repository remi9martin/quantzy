...
function Card({ m }) {
  ...
  return (
    <div className={`hairline rounded-2xl bg-white p-4 relative overflow-hidden ${m.tier === 'apex' ? 'gold-frame' : ''}`}>
      ...
      <div className="text-[11.5px] text-slate2">You: <span className="text-ink font-medium">{m.action}</span> · reason: <span className="text-warn">{m.reason}</span></div>

      <div className="mt-3 grid grid-cols-3 gap-2">
        <Box k="Edge then" v={`+${m.edgeAtTime}%`} c="text-ink" />
      ...
      <div className="mt-3">
        ...
        <div className="flex items-center justify-between text-[10.5px] text-slate2 mt-1">
...
function Box({ k, v, c }) {
  return <div className="hairline rounded-md p-2 bg-platinum/40"><div className="eyebrow">{k}</div><div className={`font-semibold tnum ${c}`}>{v}</div></div>;
}
...
export function Missed() {
  return (
    <div className="px-4 lg:px-6 py-5 max-w-[1600px] mx-auto space-y-5">
      ...
      <div className="hairline rounded-2xl bg-gradient-to-br from-[#fff7ec] via-white to-white p-5">
        ...
        <div className="grid lg:grid-cols-3 gap-3 mt-3">
          {[
            ...
            <div key={i} className="hairline rounded-md p-3 bg-white">
              <div className="text-[12.5px] text-ink">{d}</div>
      ...
      <div className="hairline rounded-2xl bg-white p-5">
        ...
        <ul className="mt-3 space-y-2">
          <li className="text-[12.5px]"><span className="text-slate2">A user searched</span> <span className="font-semibold">"AI regulation"</span> <span className="text-slate2">9 days before it moved</span> <span className="text-positive font-semibold">+31¢</span>.</li>
          <li className="text-[12.5px]"><span className="text-slate2">Quantzy flagged</span> <span className="font-semibold">"BTC spot ETF"</span> <span className="text-slate2">before the crowd by 4 days.</span></li>
          <li className="text-[12.5px]"><span className="text-slate2">Apex users captured</span> <span className="font-semibold">$24k</span> <span className="text-slate2">on the missed Q1 macro print.</span></li>
        </ul>
      </div>
    </div>
  );
