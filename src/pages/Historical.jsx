...
function CalibrationDiagram({ data }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full">
      {[60, 70, 80, 90].map((g) => (
        <g key={g}>
          <line x1={x(g)} x2={x(g)} y1={20} y2={H - P} stroke="rgba(11,13,16,0.05)" />
          <line y1={y(g)} y2={y(g)} x1={P} x2={W - 16} stroke="rgba(11,13,16,0.05)" />
      ...
      {data.map((d, i) => (
        <g key={i}>
          <circle cx={x(d.predicted)} cy={y(d.actual)} r="6" fill="#fff" stroke="#2563EB" strokeWidth="2.5" />
          <text x={x(d.predicted)} y={y(d.actual) - 12} textAnchor="middle" fontSize="9.5" fill="#0B0D10" fontWeight="600">n={d.n}</text>
        </g>
      ))}
  ...
  return (
    <div className="px-4 lg:px-6 py-5 max-w-[1600px] mx-auto space-y-5">
      <div>
        ...
        <p className="text-[13px] text-slate2 mt-0.5 max-w-[60ch]">Every confidence band is benchmarked against the actual resolution rate of every signal in that band. We publish the math and let it stand on its own.</p>
      </div>
      ...
      <div className="hairline rounded-2xl bg-white">
        <div className="flex items-center justify-between p-4 border-b border-mist">
          ...
          <span className="text-[11px] text-slate2">Last refresh · 2h ago</span>
        </div>
        <table className="w-full text-[12px]">
          ...
          <tbody>
            {CALIB.map((r) => (
              <tr key={r.bucket} className="border-t border-mist">
          ...
          <ul className="mt-3 space-y-2">
            {CATEGORIES.map((c) => (
              <li key={c.topic} className="grid grid-cols-[140px_minmax(0,1fr)_120px] items-center gap-3 text-[12px]">
                ...
                <span className="tnum text-slate2 text-right">+{c.edge}% edge · {c.hit}% hit · {c.samples}</span>
              </li>
            ))}
          </ul>
        </div>
      ...
      <div className="hairline rounded-2xl bg-white">
        <div className="flex items-center justify-between p-4 border-b border-mist">
          ...
          <span className="text-[11px] text-slate2">Showing last 12</span>
        </div>
          ...
          <tbody>
            {SIGNALS.map((s, i) => (
              <tr key={s.id} className="border-t border-mist">
                <td className="py-1.5 px-4 tnum text-slate2">2026-04-{(i + 12).toString().padStart(2, '0')}</td>
...
function Tile({ k, v, c = 'text-ink' }) {
  return (
    <div className="hairline rounded-xl bg-white p-3.5">
