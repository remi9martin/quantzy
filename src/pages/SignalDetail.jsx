      ...
      <div className="grid lg:grid-cols-[1.4fr_1fr] gap-5">
        ...
        <div className="space-y-5">
          <div className="hairline rounded-2xl bg-white p-5">
            ...
            <div className="mt-3 grid grid-cols-3 gap-2 text-[11.5px]">
              <li className="hairline rounded-md p-2 bg-platinum/40 list-none"><div className="eyebrow">News sentiment</div><div className="font-semibold tnum">+0.62</div></li>
              <li className="hairline rounded-md p-2 bg-platinum/40 list-none"><div className="eyebrow">Market flow</div><div className="font-semibold tnum">+0.18</div></li>
              <li className="hairline rounded-md p-2 bg-platinum/40 list-none"><div className="eyebrow">Liquidity stress</div><div className="font-semibold tnum">-0.04</div></li>
            </div>
          </div>
          ...
          <div className="hairline rounded-2xl bg-white p-5">
            ...
            <ul className="text-[12.5px] mt-2 grid sm:grid-cols-2 gap-2">
              <li className="hairline rounded-md p-2 bg-platinum/40">Price &gt; 0.62 (model exit)</li>
              <li className="hairline rounded-md p-2 bg-platinum/40">Liquidity collapse below $400k</li>
              <li className="hairline rounded-md p-2 bg-platinum/40">Resolution rule challenged</li>
              <li className="hairline rounded-md p-2 bg-platinum/40">7d implied vol &gt; 80th pct</li>
            </ul>
          </div>
        </div>
        ...
        <div className="space-y-5">
          <div className="hairline rounded-2xl bg-white p-5">
            ...
            <div className="text-[18px] font-display tnum mt-1">{s.socialConv}% <span className="text-[12px] text-slate2 font-sans">composite conviction</span></div>
            <div className="flex -space-x-1.5 mt-2">
              {TRADERS.slice(0, 6).map((t) => (
                <span key={t.name} title={`@${t.name}`} className="h-7 w-7 rounded-full bg-ink text-white grid place-items-center text-[10px] font-semibold ring-2 ring-white">
            ...
            <ul className="mt-3 text-[12px] space-y-1.5">
              ...
              <li className="flex items-center justify-between"><span>{s.traders.skipping} skipping</span><span className="text-slate2">— neutral</span></li>
          ...
          <div className="hairline rounded-2xl bg-white p-5">
            ...
            <ul className="space-y-2">
              {related.map((r) => (
                <li key={r.id} className="hairline rounded-md p-2 bg-platinum/30">
          ...
          <div className="hairline rounded-2xl bg-white p-5">
            ...
            <ul className="mt-2 text-[12px] text-slate2 space-y-1">
          ...
          <div className="hairline rounded-2xl bg-gradient-to-br from-[#fffaee] to-white p-5 gold-frame">
            ...
            <div className="text-[12.5px] text-slate2 mt-1">Export a Quantzy-branded card with your call, confidence, and outcome window. Auto-mints to your profile if you take it.</div>
...
function Box({ k, v, c = 'text-ink', sub }) {
  return (
    <div className="hairline rounded-md p-2.5 bg-platinum/40">
      <div className="eyebrow">{k}</div>
      <div className={`font-semibold tnum text-[18px] ${c}`}>{v}</div>
      {sub && <div className="text-[10.5px] text-slate2">{sub}</div>}
    </div>
  );
}
  ...
  return (
    <div>
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full">
        <line x1={P} y1={y(0)} x2={W - P} y2={y(0)} stroke="rgba(11,13,16,0.10)" />
        {components.map((c, i) => {
          const yStart = y(cum);
          cum += c.v;
          const yEnd = y(cum);
          ...
          return (
            <g key={c.k}>
              ...
              <text x={x(i) + ((W - P * 2) / components.length) / 2} y={top - 5} textAnchor="middle" fontSize="10" fontWeight="600" fill="#0B0D10">
        ...
        <g>
          <rect x={W - P - 70} y={y(cum) - 18} width="68" height="18" rx="9" fill="#0B0D10" />
